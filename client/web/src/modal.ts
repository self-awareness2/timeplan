...
export function showItemModal(item: ScheduleItem | null, defaultDate: string, callbacks: ModalCallbacks): void {
  ...
  const save = async () => {
    ...
    try {
      if (isEdit && item) await api.updateItem(item.id, form); else await api.addItem(form);
  ...
  overlay.querySelector('[data-delete]')?.addEventListener('click', async () => { if (item && callbacks.onDelete && confirm('确定删除这条计划吗？')) { await api.deleteItem(item.id); close(); callbacks.onDelete(item.id); } });
